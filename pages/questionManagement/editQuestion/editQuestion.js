layui.use(['layer', 'form'], function() {
    ...
    renderQuestion([{ question_type_id: 1, options: [1, 2, 4] }, { question_type_id: 2, options: [1, 2, 4] }, { question_type_id: 3 }])

    /**
     * 渲染问卷
     *
     * @param {*} question 问卷题目等数据
     */
            ...
            switch (item.question_type_id) {
                case 1:
                    html += `<div class="layui-form-item">
                                ...
                                <div class="item">
                                    <input type="text" name="radio_${subjectCount}" class="layui-input" value="${item.question_type_id}">`;
                    item.options.forEach((option, index) => {
                        html += `   <div class="options">
                                        <input type="radio">
                                        <input type="text" name="radio_${subjectCount}_${index + 1}" class="layui-input" value="${item.question_type_id + index + 1}">
                                        <button type="button" class="layui-btn layui-btn-primary layui-btn-xs del_option" data-delOpId="${index + 1}">删除</button>
                                    </div>`
                    });
                    html += `       <div class="optionButton">
                                        <button type="button" class="layui-btn layui-btn-xs add_option" data-addOpId="radio-${subjectCount}-${item.options.length}">添加选项</button>
                                    </div>
                                </div>
                ...
                case 2:
                    html += `<div class="layui-form-item">
                                <label class="layui-form-label">${++subjectCount}.</label>
                                <div class="item">
                                    <input type="text" name="checkbox_${subjectCount}" class="layui-input" value="${item.question_type_id}">`;
                    item.options.forEach((option, index) => {
                        html += `   <div class="options">
                                        <input type="checkbox" lay-skin="primary">
                                        <input type="text" name="checkbox_${subjectCount}_${index + 1}" class="layui-input" value="${item.question_type_id + index + 1}">
                                        <button type="button" class="layui-btn layui-btn-primary layui-btn-xs del_option" data-delOpId="${index + 1}">删除</button>
                                    </div>`
                    });
                    html += `       <div class="optionButton">
                ...
                case 3:
                    html += `<div class="layui-form-item">
                                <label class="layui-form-label">${++subjectCount}.</label>
                                <div class="item">
                                    <input type="text" name="input_${subjectCount}" class="layui-input" value="${item.question_type_id}">
                                    <div class="options">
    ...
    $('#back_edit').click(function() {
        ...
        window.sessionStorage.removeItem('qId');
